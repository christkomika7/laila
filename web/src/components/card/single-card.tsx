...
import { Check, Music2, ShoppingCart, X } from "lucide-react";
...
export const SingleCard = ({ track }: SingleCardProps) => {
  const items = useCartStore.use.items();
  const addTrack = useCartStore.use.addTrack();
  const removeItem = useCartStore.use.removeItem();

  const cartKey = `track:${track.id}`;
  const trackInCart = items.some((i) => i.id === cartKey);

  const coverUrl = track.coverUrl ?? FALLBACK_IMAGE;
  const releaseYear = track.album
  ...
  const artists =
    track.featuringArtists.length > 0
      ? `feat. ${track.featuringArtists.join(", ")}`
      : null;

  const handleAddToCart = (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    if (trackInCart) return;
    addTrack(track);
  ...
  return (
    <PremiumCard className="flex flex-col h-full group">
      <div className="relative aspect-square overflow-hidden">
        ...
        <span className="absolute top-3 left-3 inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold uppercase tracking-wide backdrop-blur-sm shadow bg-secondary/80 text-secondary-foreground border border-secondary">
      ...
      <div className="p-6 flex flex-col grow">
        <h3 className="text-xl font-bold mb-1 line-clamp-1">{track.title}</h3>
        <p className="text-muted-foreground text-sm mb-4 line-clamp-1">
          Laila{artists ? ` ${artists}` : ""}
          {releaseYear ? ` • ${releaseYear}` : ""}
        </p>

        <div className="mt-auto pt-4 flex items-center justify-between gap-2">
          <span className="text-xl font-bold text-primary whitespace-nowrap">
            {track.price > 0 ? priceDisplay : "Gratuit"}
          </span>
