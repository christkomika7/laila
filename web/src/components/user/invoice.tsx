...
export default function UserInvoiceTab() {
  ...
  const fetchInvoices = async () => {
    setLoading(true);
    try {
      const res = await apiServerClient.fetch("/invoices/me", {
        credentials: "include",
      });
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await apiServerClient.fetch(
        `/invoices/me/${deleteTarget.type}/${deleteTarget.id}`,
        { method: "DELETE", credentials: "include" },
      );
      if (!res.ok && res.status !== 204) throw new Error();
      // hide passe à true côté serveur -> on retire l'élément de la liste affichée
      setInvoices((prev) =>
        prev.filter(
          (inv) =>
