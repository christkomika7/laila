import { WORLD_COUNTRIES } from "#/lib/data";
import { AnimatePresence, motion } from "framer-motion";
import { ChevronDown, Search, X } from "lucide-react";
import { useEffect, useRef, useState } from "react";

export const WorldCountryPicker = ({
  value,
  onChange,
  disabled,
}: {
      ...
      >
        <span className="flex items-center gap-3">
          {selected ? (
            ...
            <span className="text-neutral-500">Sélectionner un pays…</span>
          )}
        </span>
        <ChevronDown
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map((c) => (
                  <button
                    ...
                    className={`w-full px-4 py-2.5 flex items-center gap-3 text-sm hover:bg-neutral-800 transition-colors text-left ${value === c.code ? "bg-neutral-800" : ""}`}
                  >
